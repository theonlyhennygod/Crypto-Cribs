...
import { Client, Wallet, Payment, TransactionMetadata, dropsToXrp, xrpToDrops } from "xrpl"
...
export function XRPLProvider({ children }: { children: ReactNode }) {
  ...
  const sendPayment = async (destination: string, amount: string, memo?: string): Promise<string> => {
    try {
      if (!state.client || !state.wallet) {
        throw new Error("Wallet not connected")
      }
      ...
      const response = await state.client.submitAndWait(payment, { wallet: state.wallet })
        ...
        .map((tx: any, index: number) => {
          ...
          return {
            ...
            memo: tx.tx.Memos?.[0]?.Memo?.MemoData 
              ? Buffer.from(tx.tx.Memos[0].Memo.MemoData, "hex").toString("utf8")
              : undefined,
            timestamp: (tx.tx.date || 0) * 1000,
            status: tx.meta?.TransactionResult === "tesSUCCESS" ? "validated" : "failed",
          }
        })
  ...
  return <XRPLContext.Provider value={contextValue}>{children}</XRPLContext.Provider>
}
